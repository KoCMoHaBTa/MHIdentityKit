...
class RequestAuthorizerAsyncTests: XCTestCase {
    ...
    func testHTTPBasicAuthorizerAsync() async {
        ...
        let urlRequest = try? await authorizer.authorizeAsync(request: request)
                
        XCTAssertEqual(urlRequest?.value(forHTTPHeaderField: "Authorization"), "Basic dHVuOnRwcw==")
    }
    
    func testBearerAccessTokenAuthorizerUsingHeaderAsync() async {
    ...
    func testBearerAccessTokenAuthorizerUsingBodyAsync() async {
        ...
        do {
            ...
            let urlRequest2 = try await authorizer.authorizeAsync(request: request2)
        ...
        let urlRequest3 = try? await authorizer.authorizeAsync(request: request3)
        ...
        let urlRequest4 = try? await authorizer.authorizeAsync(request: request4)
        ...
        let urlRequest5 = try? await authorizer.authorizeAsync(request: request5)
        
        XCTAssertEqual(urlRequest5?.httpBody?.base64EncodedString(), "dGRwPXRkdiZhY2Nlc3NfdG9rZW49dGVzdF90b2tlbg==")
    }
    
    func testBearerAccessTokenAuthorizerUsingQueryAsync() async {
